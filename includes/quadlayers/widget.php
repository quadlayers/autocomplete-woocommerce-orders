...
if ( ! class_exists( 'QL_Widget' ) ) {
	...
	class QL_Widget {
		...
		<div>
			<div>
				<div style="margin-top: 11px;float: left;width: 70%;">
					...
					<div style="margin-top: 11px; float: left; width: 70%;"><a href="<?php echo admin_url( 'admin.php?page=' . ACO_PREFIX . '_suggestions' ); ?>" target="_blank" class="button button-secondary"><?php esc_html_e( 'More products', 'woocommerce-checkout-manager' ); ?></a></div>
				</div>
			...
			<ul>
				<?php if ( is_array( $posts ) ) { ?>
					...
					foreach ( $posts  as $post ) {
						...
						printf( __( '<li><strong><a href="%1$s" target="_blank">%2$s</a></strong>%3$s%4$s</li>', 'woocommerce-checkout-manager' ), $link, $title, $date, $summary );
					}
					?>
					<?php
				} else {
					printf( __( '<li>%s</li>', 'woocommerce-checkout-manager' ), $posts );
				}
				?>
			</ul>
		</div>
		<div style="display: flex; justify-content: space-between;align-items: center;margin: 16px -12px 0;padding: 12px 12px 0; border-top: 1px solid #eee;">
			<a href="<?php printf( 'https://quadlayers.com/blog/?utm_source=%s&utm_medium=software&utm_campaign=wordpress&utm_content=dashboard', ACO_PREFIX ); ?>" target="_blank"><?php esc_html_e( 'Read more like this on our blog', 'woocommerce-checkout-manager' ); ?></a>
			<a class="button-primary" href="<?php printf( 'https://quadlayers.com/?utm_source=%s&utm_medium=software&utm_campaign=wordpress&utm_content=dashboard', ACO_PREFIX ); ?>" target="_blank"><?php esc_html_e( 'QuadLayers', 'woocommerce-checkout-manager' ); ?></a>
		</div>
			<?php
		}
